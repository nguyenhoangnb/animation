...
class EyeWidget(QWidget):
    def __init__(self, eyes_data, parent=None):
        ...
        self.blink_interval = 2000  # Nháy mắt mỗi 2 giây
        ...
        for layer in self.eyes_data:
            # Xử lý position cho cả static và animated
            pos_data = layer["ks"]["p"]
            if pos_data.get("a", 0) == 1 and isinstance(pos_data["k"], list):
                # Animated: lấy giá trị 's' của keyframe đầu tiên
                pos_x, pos_y = pos_data["k"][0]["s"]
            else:
                # Static: lấy trực tiếp
                pos_x, pos_y = pos_data["k"]
            pos_x *= scale_x
            pos_y *= scale_y
            name = layer["nm"].lower()
            ...
            if "eye" in name and len(self.eye_info) < 2:  # Limit to 2 eyes
                ...
                # Lưu keyframes nếu có animation
                pos_data = layer["ks"]["p"]
                if pos_data.get("a", 0) == 1 and isinstance(pos_data["k"], list):
        ...
        self.blink_progress = 1.0  # 1.0: mở hoàn toàn, 0.0: nhắm hoàn toàn
        ...
        self.blink_anim_direction = -1  # -1: nhắm, 1: mở
    ...
    def paintEvent(self, event):
        ...
        for eye in self.eye_info:
            ...
            eye_height = eye["eye_height"] * self.blink_progress  # Thay đổi chiều cao theo blink_progress
            outline_color = eye["outline_color"]
            fill_color = eye["fill_color"]

            if self.blink_progress > 0.05:
                # Draw open/closing eye (outline and fill)
                ...
                # Draw closed eye (a horizontal line)
                painter.setPen(QPen(outline_color, 2))
                painter.setBrush(Qt.NoBrush)
                painter.drawLine(int(pos_x - eye_width // 2), int(pos_y), int(pos_x + eye_width // 2), int(pos_y))

        current_frame = int((time.time() * 60) % 60)  # 60 fps, lặp lại mỗi 60 frame
        # Draw pupils (only when eyes are open)
        if not self.is_blinking:
            for pupil in self.pupil_info:
                keyframes = pupil["keyframes"]
                pos_x, pos_y = self.get_pupil_pos(keyframes, current_frame, 800/150, 480/150)
...
class ScreensaverManager:
    def __init__(self, parent, container):
        ...
        # Nạp toàn bộ layer từ eye_animation.json (KHÔNG lấy pupil từ eye_change.json)
        try:
...
def main():
    ...
    stacked_widget.resize(800, 480)  # Set fixed size instead of full screen
    stacked_widget.show()            # Show windowed mode
    sys.exit(app.exec_())
